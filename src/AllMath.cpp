...
vec3d AllMath::vectorIntersectPlane(vec3d &plane_p, vec3d &plane_n, vec3d &lineStart, vec3d &lineEnd){
    plane_n = AllMath::norm(plane_n);
    float plane_d = - AllMath::dotProd(plane_n, plane_p);
    float ad = AllMath::dotProd(lineStart, plane_n);
    float bd = AllMath::dotProd(lineEnd, plane_n);
    float t = (-plane_d - ad) / (bd - ad);
    vec3d lineStartToEnd = AllMath::subVector(lineEnd, lineStart);
    vec3d lineToIntersect = AllMath::mulVector(lineStartToEnd, t);
    return AllMath::addVector(lineStart, lineToIntersect);
}

int AllMath::triangleClipAgainstPlane(vec3d plane_p, vec3d plane_n, triangle &in_tri, triangle &out_tri1, triangle &out_tri2){
    // Make sure plane normal is indeed normal
    plane_n = AllMath::norm(plane_n);

    // Return signed shortest distance from point to plane, plane normal must be normalised
    auto dist = [&](vec3d &p){
        vec3d n = AllMath::norm(p);
        return (plane_n.x * p.x + plane_n.y * p.y + plane_n.z * p.z - AllMath::dotProd(plane_n, plane_p));
    };
    ...
    if (nInsidePointCount == 1 && nOutsidePointCount == 2){
        ...
        out_tri1.p[1] = AllMath::vectorIntersectPlane(plane_p, plane_n, *inside_points[0], *outside_points[0]);
        out_tri1.p[2] = AllMath::vectorIntersectPlane(plane_p, plane_n, *inside_points[0], *outside_points[1]);
    ...
    if (nInsidePointCount == 2 && nOutsidePointCount == 1){
        ...
        out_tri1.p[2] = AllMath::vectorIntersectPlane(plane_p, plane_n, *inside_points[0], *outside_points[0]);
        ...
        out_tri2.p[2] = AllMath::vectorIntersectPlane(plane_p, plane_n, *inside_points[1], *outside_points[0]);
